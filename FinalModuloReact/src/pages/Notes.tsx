...
const Notes: React.FC = () => {
    ...
    const handleDeleteNote = (note: NoteType) => {
        if (confirm(`Sua nota 'Assunto: "${note.description}" e Detalhe: "${note.detail}"' serão excluidas!`) == true) {
    ...
    const handSaveEditedNote = () => {
        if (editDescription.length <= 3) {
            alert(`Assunto: "${editDescription}"invalido, mínimo 4 caracteres.`);
        } else if (editDetail.length <= 3) {
            alert(`Conteudo: "${editDetail}"invalido, minimo 4 caracteres.`);
        } else {
    ...
    return (
        <React.Fragment>
            ...
            <Dialog open={openEdit} onClose={handleClose}>
                ...
                <DialogContent>
                    <DialogContentText>Campos para novos Assuntos e Conteudos!</DialogContentText>
                    <TextField
                        autoFocus
                        margin="dense"
                        label="Assunto"
